...
function StatusBadge({ value }) 
{
  return (
    <span
      className={`status-badge status-${String(value || "other")
        .toLowerCase()
        .replaceAll("_", "-")}`}
    >
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                key={optionValue}
                onClick={() => {
                  multiple ? onToggle(optionValue) : onSelect(optionValue);
                  if (!multiple) setOpen(false);
                }}
              >
                <span>{optionLabel}</span>
                {checked ? <Icon name="activate" /> : null}
              </button>
            );
          })}
        </div>
    ...
    <div className="drawer-backdrop" role="presentation">
      <aside className="user-drawer sold-detail-drawer">
        ...
        <div className="drawer-profile">
          <h3>{order.ebay_account_name}</h3>
          <p>
            {order.buyer_username || "Buyer hidden"} - {dt(order.creation_date)}
          </p>
          <StatusBadge value={order.normalized_status} />
        </div>
        <section className="drawer-section">
          <h3>Payment</h3>
          <p>Status: {order.order_payment_status || "-"}</p>
          <p>Paid: {dt(order.payment_date)}</p>
          <p>
            Total: {money(order.order_total, order.currency)} - Due seller:{" "}
            {money(order.total_due_seller, order.currency)}
          </p>
        </section>
        <section className="drawer-section">
          <h3>Fulfillment</h3>
          <p>Status: {order.order_fulfillment_status || "-"}</p>
          <p>
            Carrier: {order.shipping_carrier_code || "-"} - Service:{" "}
            {order.shipping_service_code || "-"}
          </p>
          <p>Tracking: {order.tracking_number || "-"}</p>
          <p>Ship by: {dt(order.ship_by_date)}</p>
        </section>
        <section className="drawer-section">
          <h3>Line Items</h3>
          {(order.line_items || []).map((item) => (
            <p key={item.id}>
              {item.title || "Untitled"} - {item.condition || "No condition"} -{" "}
              <CopyValue value={item.sku || "No SKU"} /> - Qty{" "}
              {item.quantity || 0} -{" "}
              {money(item.line_item_total, item.currency)}
            </p>
          ))}
        </section>
      </aside>
...
function EditSoldPostingModal({
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section
        className="modal-panel sold-edit-modal"
        role="dialog"
        aria-modal="true"
      >
...
export default function SoldPosting({ currentUser, onLogout }) {
  ...
  useEffect(() => {
    fetchSoldPostingOptions()
      .then(setOptions)
      .catch(() => setOptions({ accounts: [], statuses: [] }));
  }, []);
  useEffect(() => {
    const timer = window.setTimeout(() => load(filters), 300);
  ...
  async function openDetail(orderId) {
    setSelected(await fetchSoldPostingDetail(orderId));
  }
  function openEdit(row, event) {
    ...
    setEditTarget({
      ...
      ship_by_date: row.ship_by_date
        ? new Date(row.ship_by_date).toISOString().slice(0, 16)
        : "",
  ...
  function updateEditField(key, value) {
    setEditTarget((current) => ({ ...current, [key]: value }));
  }
  async function saveEdit() {
    ...
    try {
      await updateSoldPostingLineItem(editTarget.id, {
        ...
        quantity:
          editTarget.quantity === "" ? null : Number(editTarget.quantity),
        ...
        ship_by_date: editTarget.ship_by_date
          ? new Date(editTarget.ship_by_date).toISOString()
          : null,
  ...
  async function syncLatest() {
    ...
    try {
      ...
      setPartialError(
        failures.length
          ? `${failures.length} account sync failed. Admin details are shown below.`
          : "",
      );
      await load(filters);
    } catch (err) {
      ...
      <main className="management-page sold-posting-page">
        <div className="page-header">
          <div>
            <h1>Sold Posting</h1>
            <p>
              {data.summary?.line_item_count || 0} matching line items -{" "}
              {data.summary?.order_count || 0} orders -{" "}
              {data.summary?.quantity_sold || 0} quantity sold - Last sync{" "}
              {dt(data.sync?.last_successful_sync_at)}
            </p>
          </div>
          <button
        ...
        {error ? <p className="form-message error">{error}</p> : null}
        {partialError ? (
          <p className="form-message error">{partialError}</p>
        ) : null}
        {syncResult && isAdmin ? (
          <section className="offer-bulk-bar">
            {syncResult.results.map((item) => (
              <span key={item.account_id}>
                {item.account_name}:{" "}
                {item.success
                  ? `${item.orders_received} orders, ${item.pages_fetched} pages`
                  : item.error_message}
              </span>
            ))}
          </section>
        ...
        <section className="stats-grid">
          {[
            ...
            ["Sold Items", data.summary?.line_item_count],
            ["Awaiting Shipment", data.summary?.awaiting_shipment],
        ...
        <form
          className="sold-filter-bar compact-sold-filter-bar"
        ...
        >
          ...
          <FilterDropdown
            ...
            onToggle={(value) => toggleMulti("account_ids", value)}
          />
          <FilterDropdown
            label="Period"
            value={
              (periodOptions.find(([value]) => value === period) || [
                period,
                period,
              ])[1]
            }
          ...
          {period === "custom" ? (
            <div className="sold-custom-period">
              <label className="sold-date-filter">
                <span>From</span>
                <input
                  type="date"
                  value={customRange.date_sold_from}
                  onChange={(event) =>
                    updateCustomRange("date_sold_from", event.target.value)
                  }
                />
              </label>
              <label className="sold-date-filter">
                ...
                <input
                  type="date"
                  value={customRange.date_sold_to}
                  onChange={(event) =>
                    updateCustomRange("date_sold_to", event.target.value)
                  }
                />
              </label>
              <button
                className="sold-custom-apply"
                type="button"
          ...
          <FilterDropdown
            label="Search by"
            value={
              (searchOptions.find(([value]) => value === searchBy) || [
                searchBy,
                searchBy,
              ])[1]
            }
        ...
        <section className="table-card">
          {loading ? (
            <div className="empty-state">Loading sold items...</div>
          ) : data.items.length ? (
            <div className="table-scroll sold-table-scroll">
              <table className="users-table sold-table">
                <thead>
                  <tr>
                    {[
                      "Copy",
                    ...
                    <tr key={row.id} onClick={() => openDetail(row.order_id)}>
                      <td className="sold-copy-cell">
                        <div className="sold-copy-actions">
                          <span
                            className={`copy-state-dot ${row.is_copied ? "copied" : "fresh"}`}
                            title={
                              row.is_copied
                                ? `Copied${row.copied_at ? ` ${dt(row.copied_at)}` : ""}`
                                : "Not copied"
                            }
                          />
                          <button
                            className="icon-button sold-copy-button"
                            type="button"
                            title={soldReferenceText(row)}
                            aria-label="Copy sold reference"
                            onClick={(event) => copySoldReference(row, event)}
                          >
                            <Icon name="copy" />
                          </button>
                          {copiedToastId === row.id ? (
                      ...
                      <td>
                        {row.item_id ? (
                          <a
                            href={`https://www.ebay.com/itm/${row.item_id}`}
                            target="_blank"
                            rel="noreferrer"
                          ...
                          >
                            {row.image_url ? (
                              <img
                                className="sold-thumb"
                                src={row.image_url}
                                alt=""
                              />
                            ) : (
                              <span className="sold-thumb placeholder">
                                <Icon name="bag" />
                              </span>
                            )}
                          </a>
                      ...
                      <td>{money(row.shipping, row.currency)}</td>
                      <td>{money(row.total, row.currency)}</td>
                      <td>{dt(row.date_sold)}</td>
                      <td>{dt(row.date_paid)}</td>
                      <td>
                        <CopyValue value={row.item_id} />
                      </td>
                      <td>
                        <button
                          className="icon-button offer-action-icon offer-action-edit"
                          type="button"
                          title="Edit order"
            ...
            <div className="empty-state">
              No sold items match these filters.
            </div>
          )}
          <div className="pagination-bar">
            <span>
              Showing {data.items.length} of {data.total || 0}
            </span>
            <div>
              <button
