...
export default function OfferManagement({ currentUser, onLogout }) {
  ...
  const [filters, setFilters] = useState({ page: 1, page_size: 25, sort_by: 'updated_at', sort_order: 'desc' })
  const [data, setData] = useState({ items: [], total: 0, page: 1, page_size: 25 })
  const [summary, setSummary] = useState({})
  const [lookups, setLookups] = useState({ statuses: [], outcomes: [], currencies: [] })
  ...
  const [error, setError] = useState('')
  const activeFilters = useMemo(() => ({ ...filters, page: undefined, page_size: undefined, sort_by: undefined, sort_order: undefined }), [filters])

  async function load(next = filters) {
    try {
      const [list, counts] = await Promise.all([fetchOfferEntries(next), fetchOfferSummary(activeFilters)])
      setData(list); setSummary(counts); setError('')
    } catch (err) { setError(err.message) }
  }

  useEffect(() => { Promise.allSettled([fetchOfferLookups(), fetchEbayAccounts(), isAgent ? Promise.resolve({ items: [] }) : fetchUsers()]).then(([l, a, u]) => { if (l.status === 'fulfilled') setLookups(l.value); if (a.status === 'fulfilled') setAccounts(a.value.items || a.value || []); if (u.status === 'fulfilled') setUsers(u.value.items || u.value || []) }) }, [])
  useEffect(() => { const timer = window.setTimeout(() => load(filters), 300); return () => window.clearTimeout(timer) }, [filters])

  function updateFilter(key, value) { setFilters((current) => ({ ...current, [key]: value, page: 1 })) }
  async function download() { const blob = await exportOfferEntries(activeFilters); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `offer-entries_${new Date().toISOString().slice(0, 10)}.xlsx`; link.click(); URL.revokeObjectURL(url) }
  async function preview(entry) { setSelected(entry); try { setHistory(isAgent ? [] : await fetchOfferHistory(entry.id)) } catch { setHistory([]) } }
  function afterSave(saved, previewSaved) { setShowCreate(false); setModalEntry(null); load(); if (previewSaved) preview(saved) }
  ...
  async function confirmDelete() {
    ...
    try {
      await deleteOfferEntry(deleteTarget.id)
  ...
  return <AppLayout activePage="Offer Management" currentUser={currentUser} onLogout={onLogout}><main className="management-page offer-management-page"><div className="page-header"><div><h1>Offer Management</h1><p>Track, follow up and manage eBay offers</p></div><div className="page-header-actions"><button className="primary-button compact-action" type="button" onClick={() => setShowCreate(true)}>New Offer Entry</button><button className="secondary-button compact-action" type="button" onClick={download}>Export Excel</button></div></div>{error ? <p className="form-message error">{error}</p> : null}<section className="stats-grid">{[['Total Entries', summary.total_entries], ['Open Offers', summary.open_offers], ['Follow-ups Due', summary.follow_ups_due], ['Awaiting Payment', summary.awaiting_payment], ['Sold', summary.sold], ['High-Value Offers', summary.high_value_offers]].map(([title, value]) => <article className="stat-card" key={title}><div><p>{title}</p><strong>{value || 0}</strong></div></article>)}</section><form className="analytics-filter-bar offer-filter-bar" onSubmit={(event) => event.preventDefault()}>{[['from_date', 'From', 'date'], ['to_date', 'To', 'date'], ['buyer_id', 'Buyer ID', 'text'], ['sku', 'SKU', 'text'], ['listing_id', 'Listing ID', 'text'], ['search', 'Search', 'search']].map(([key, title, type]) => <label className="field" key={key}><span>{title}</span><input type={type} value={filters[key] || ''} onChange={(event) => updateFilter(key, event.target.value)} /></label>)}{!isAgent ? <label className="field"><span>Agent</span><select value={filters.created_by_user_id || ''} onChange={(event) => updateFilter('created_by_user_id', event.target.value)}><option value="">All</option>{users.map((user) => <option key={user.id} value={user.id}>{user.full_name || user.email}</option>)}</select></label> : null}<label className="field"><span>Account</span><select value={filters.ebay_account_id || ''} onChange={(event) => updateFilter('ebay_account_id', event.target.value)}><option value="">All</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.account_name || account.store_name || account.ebay_username}</option>)}</select></label><label className="field"><span>Status</span><select value={filters.status || ''} onChange={(event) => updateFilter('status', event.target.value)}><option value="">All</option>{lookups.statuses.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field"><span>Outcome</span><select value={filters.outcome || ''} onChange={(event) => updateFilter('outcome', event.target.value)}><option value="">All</option>{lookups.outcomes.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field"><span>High Value</span><select value={filters.is_high_value ?? ''} onChange={(event) => updateFilter('is_high_value', event.target.value)}><option value="">All</option><option value="true">Yes</option><option value="false">No</option></select></label><label className="field"><span>Currency</span><select value={filters.currency || ''} onChange={(event) => updateFilter('currency', event.target.value)}><option value="">All</option>{lookups.currencies.map((item) => <option key={item}>{item}</option>)}</select></label><button className="secondary-button compact-action" type="button" onClick={() => setFilters({ page: 1, page_size: 25, sort_by: 'updated_at', sort_order: 'desc' })}>Clear filters</button></form>{canDelete ? <section className="offer-bulk-bar">{selectedEntryIds.size ? <><strong>{selectedEntryIds.size} selected</strong><button className="secondary-button compact-action" type="button" disabled={isBulkDeleting} onClick={confirmBulkDelete}>{isBulkDeleting ? 'Deleting...' : 'Delete selected'}</button><button className="secondary-button compact-action" type="button" onClick={() => setSelectedEntryIds(new Set())}>Clear selection</button></> : <span>Select entries to bulk delete.</span>}</section> : null}<section className="table-card"><div className="table-scroll"><table className="users-table offer-table"><thead><tr>{canDelete ? <th><input type="checkbox" aria-label="Select all visible entries" checked={data.items.length > 0 && data.items.every((entry) => selectedEntryIds.has(entry.id))} onChange={(event) => toggleSelectAllVisible(event.target.checked)} /></th> : null}{['Entry #', 'Date', ...(!isAgent ? ['Agent'] : []), 'Account', 'Listing ID', 'SKU', 'Product', 'Buyer', 'Listed Price', 'Buyer Offer', 'Best/Counteroffer', 'Quantity', 'Status', 'Follow-up', 'High Value', 'Updated At', 'Actions'].map((head) => <th key={head}>{head}</th>)}</tr></thead><tbody>{data.items.map((entry) => <tr key={entry.id}>{canDelete ? <td><input type="checkbox" aria-label={`Select entry #${entry.entry_number}`} checked={selectedEntryIds.has(entry.id)} onChange={() => toggleEntrySelection(entry.id)} /></td> : null}<td>#{entry.entry_number}</td><td>{entry.offer_date}</td>{!isAgent ? <td>{entry.agent_name || '—'}</td> : null}<td>{entry.ebay_account_name}</td><td>{entry.listing_id}</td><td>{entry.sku || '—'}</td><td className="truncate-cell" title={entry.product_title || ''}>{entry.product_title || '—'}</td><td>{entry.buyer_id || '—'}</td><td>{money(entry.listed_price, entry.currency)}</td><td>{money(entry.buyer_offer_price, entry.currency)}</td><td>{money(entry.counteroffer_price, entry.currency)}</td><td>{entry.offer_quantity || entry.listing_quantity || '—'}</td><td><Badge value={entry.status} /></td><td>{entry.follow_up_1_notes || entry.follow_up_2_notes ? 'Added' : '—'}</td><td>{entry.is_high_value ? <Badge value="Yes" tone="active" /> : '—'}</td><td>{new Date(entry.updated_at).toLocaleString()}</td><td><div className="offer-icon-actions"><ActionIcon title="Preview" icon="eye" tone="preview" onClick={() => preview(entry)} /><ActionIcon title="Edit" icon="edit" tone="edit" onClick={() => setModalEntry(entry)} />{canDelete ? <ActionIcon title="Delete" icon="trash" tone="delete" onClick={() => setDeleteTarget(entry)} /> : null}{entry.listing_url ? <ActionIcon title="Open eBay Listing" icon="external" tone="external" href={entry.listing_url} external /> : null}{entry.related_conversation_id ? <ActionIcon title="Open Related Conversation" icon="message" tone="conversation" href={`/inbox?conversation_id=${entry.related_conversation_id}`} /> : null}</div></td></tr>)}</tbody></table></div><div className="pagination-bar"><span>Showing {data.items.length} of {data.total}</span><div><button className="pagination-button" type="button" disabled={filters.page <= 1} onClick={() => setFilters((f) => ({ ...f, page: f.page - 1 }))}>Prev</button><button className="pagination-button" type="button" disabled={filters.page * filters.page_size >= data.total} onClick={() => setFilters((f) => ({ ...f, page: f.page + 1 }))}>Next</button><select value={filters.page_size} onChange={(event) => setFilters((f) => ({ ...f, page: 1, page_size: Number(event.target.value) }))}><option>25</option><option>50</option><option>100</option></select></div></div></section></main>{showCreate ? <OfferForm lookups={lookups} accounts={accounts} onCancel={() => setShowCreate(false)} onSaved={afterSave} /> : null}{modalEntry ? <OfferForm entry={modalEntry} lookups={lookups} accounts={accounts} onCancel={() => setModalEntry(null)} onSaved={afterSave} /> : null}<PreviewDrawer entry={selected} history={history} canDelete={canDelete} onClose={() => setSelected(null)} onEdit={() => { setModalEntry(selected); setSelected(null) }} onDelete={setDeleteTarget} />{deleteTarget ? <div className="modal-backdrop" role="presentation"><section className="modal-panel" role="dialog" aria-modal="true"><div className="modal-header"><h2>Delete Offer Entry</h2><button className="icon-button" type="button" onClick={() => setDeleteTarget(null)}><Icon name="close" /></button></div><p className="confirm-message">Delete entry #{deleteTarget.entry_number}? This action cannot be undone.</p><div className="modal-actions"><button className="secondary-button" type="button" onClick={() => setDeleteTarget(null)}>Cancel</button><button className="primary-button" type="button" onClick={confirmDelete}>Delete Entry</button></div></section></div> : null}</AppLayout>
}
